...
3. 처음에 한번 , 특정 state, 특정 props 업데이트 시 **
useEffect(() => {
  실행문
}, [ state , props ] )


4. 처음에 한번 , 특정 state, 특정 props 업데이트 시 **
useEffect(() => {
  실행문
...
다음 이펙트 처리 시 이전에 이펙트를 제거하고 다시 실행할 수 있도록 만들어줌

사용범위
1. 특정 props 받아서 갱신할 때
2. setInterval , setTimeout 시간처리 할 때
3. ajax - 비동기 처리할 때
4. 외부 라이브러리 사용할 때 
